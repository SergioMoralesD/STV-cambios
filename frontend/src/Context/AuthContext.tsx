...
import { buildMenuMaps, type MenuConfig, type MenuMaps } from "../config/menuConfig";
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    /**
     * Sincroniza el token en LocalStorage para servicios externos como el Logger.
     */
    const syncToken = (userData: any) => {
        // Solo guardamos si el token es una cadena real y válida
        const token = userData?.session_token || userData?.token;
        if (token && typeof token === 'string' && token !== 'undefined') {
            localStorage.setItem('token', token);
        } else {
            // Si no hay token, mejor limpiar que guardar basura
