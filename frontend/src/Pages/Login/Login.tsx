  ...
  const manejarInicio = async (e: React.SyntheticEvent) => {
    ...
    if (userEmpty || passEmpty) {
      ...
      try {
        ...
        remoteLog(`Error en login: ${message}`, { level: 'ERROR' });
        ...
        if (message === 'SESSION_ALREADY_ACTIVE') {
          ...
          showError('No se puede conectar con el servidor. Arranca el backend (puerto 4000) e inténtalo de nuevo.');
          activarAnimacion(setAnimateUser);
          activarAnimacion(setAnimatePass);
        } else {
